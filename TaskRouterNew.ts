        ...
        if(LEMMACT) {
            ...
            if (LEMMACT[0] === 'найти' || LEMMACT[0] === "какие" || LEMMACT[0] === "какой") {
                ...
                if (tasks.length > 0) {
                    ...
                    return acc + ` ${index+1}) ${i.name} в ${i.time.substring(11,16)} ${i.date.substring(8,10)} ${parseMonth(i.date.substring(5,7))}'.`
            ...
            if (LEMMACT[0] === 'кто') {
                ...
                if (missing_fields.length === 2 || missing_fields.length === 1 && !TIME) {
                    ...
                    console.log(123)
                    const REALTIME = TIME ? TIME : ['10:00']
                    const REALLEMMTIME = LEMMTIME ? LEMMTIME : ['10:00']
                    // Добавляем задачу в базу данных для указанного пользователя на указанное время и дату
                    createUserTask({...task, TIME: REALTIME, LEMMTIME: REALLEMMTIME});
                ...
                // } else {
                    ...
                    const actionAnswer = updateTask(task);
                    return actionAnswer;
                // }
            }
